...
class MultiheadAttention(nn.Module):
    ...
    def attention(q, k, v, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            print("tupleLen: ", len(mask), "0: ", mask[0].shape, "1: ", mask[1].shape)
    ...
    def forward(self, q, k, v, mask):
        ...
        # Splits query by batches and heads so each head is seq_len by d_k
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        # Pass input through self attention and feed forward layers
...
class Transformer(nn.Module):
    ...
    def forward(self, src, src_mask, tgt, tgt_mask):
        return self.project(self.decode(self.encode(src, src_mask), tgt, src_mask, tgt_mask))
    

def buildTransformer(vocab_size: int, max_src_len: int, max_tgt_len: int, d_model: int = 1600, N: int = 6, h: int = 8, dropout: float = 0.08, d_ff: int = 2048, warmup_steps: int = 2000):
    # Build embedding layers
    e_in_embedding = InputEmbedding(d_model, vocab_size)
    d_in_embedding = InputEmbedding(d_model, vocab_size)

    # Build positional embeding layers
    e_pos_embedding = PositionalEncoding(d_model, max_src_len, dropout)
    d_pos_embedding = PositionalEncoding(d_model, max_tgt_len, dropout)
